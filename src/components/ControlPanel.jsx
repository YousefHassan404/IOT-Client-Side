...
export default function ControlPanel({ deviceControls, onControl, sensorId }) {
  ...
  const handleServoAngleChange = (angle) => {
    setServoAngle(Number(angle));
    onControl(sensorId, 'servoAngle', Number(angle));
  };
      ...
      <div className="control-grid">
        ...
        <div className="control-card">
          ...
          <div className="control-buttons">
            <button
              className={`control-btn ${deviceControls?.led === 'on' ? 'active' : ''}`}
              onClick={() => handleLedControl('on')}
            >
              Turn On
            </button>
            <button
              className={`control-btn ${deviceControls?.led === 'off' ? 'active' : ''}`}
        ...
        <div className="control-card">
          ...
          <div className="control-buttons">
            <button
              className={`control-btn ${deviceControls?.buzzer === 'on' ? 'active' : ''}`}
              onClick={() => handleBuzzerControl('on')}
            >
              On
            </button>
            <button
              className={`control-btn ${deviceControls?.buzzer === 'off' ? 'active' : ''}`}
        ...
        <div className="control-card">
          ...
          <div className="control-buttons">
            <button
              className={`control-btn ${deviceControls?.servo === 'enabled' ? 'active' : ''}`}
              onClick={() => handleServoControl('enabled')}
            >
              Enable
            </button>
            <button
              className={`control-btn ${deviceControls?.servo === 'disabled' ? 'active' : ''}`}
              onClick={() => handleServoControl('disabled')}
            >
              Disable
            </button>
          </div>
